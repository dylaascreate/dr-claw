...
export default function DrawerBook({ show, onClose, preselectedParams, onConfirm }) {
  ...
  return (
    <div
      className={`absolute bottom-0 left-0 right-0 h-[85%] bg-cream rounded-t-[32px] shadow-premium z-50 transform transition-transform duration-300 ease-out flex flex-col overflow-hidden ${show ? 'translate-y-0' : 'translate-y-full'
        }`}
    >
      <div
              ...
              {SERVICES.map((s) => {
                ...
                return (
                  <button
                    ...
                    className={`p-3.5 bg-white border rounded-2xl text-left shadow-sm transition-all relative block ${isSelected ? 'border-sage-500' : 'border-brown-100 hover:border-brown-400'
                      }`}
                  >
                    {isSelected && (
              ...
              {PRACTITIONERS.map((p) => {
                ...
                return (
                  <div
                    ...
                    className={`p-3 bg-white border rounded-2xl flex items-center justify-between cursor-pointer transition-all shadow-sm ${isSelected ? 'border-sage-500' : 'border-brown-100 hover:border-brown-400'
                      }`}
                  >
                    <div className="flex items-center gap-3">
                    ...
                    <div
                      className={`w-4 h-4 rounded-full border ${isSelected ? 'border-4 border-sage-500' : 'border-brown-400'
                        }`}
                    ></div>
                  </div>
                );
                ...
                {availableTimes.map((t) => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-2.5 border rounded-xl text-xs font-semibold text-center shadow-sm transition-all ${!isAvailable
